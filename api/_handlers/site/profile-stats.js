...
module.exports = async (req, res) => {
  ...
  // الأيام المتتالية — من غير انقطاع لحد النهاردة
  ...
  // نشاط آخر 7 أيام (السبت للجمعة بترتيب تصاعدي لحد النهاردة)
  ...
  // الترتيب النسبي — بمقارنة متوسط درجات الطالب بمتوسطات باقي الطلاب
  // لو العينة صغيرة (أقل من 5 طلاب ليهم نتايج) بنرجع null عشان الرقم ميبقاش مضلل
  let percentile = null;
  if (quizzesCompleted > 0) {
  ...
  // الشهر المفتوح حاليًا وعدد الكويزات المتبقية فيه
  ...
  let allMonthsDone = false;
  {
    ...
    for (const month of months || []) {
      ...
      if (previousPassed) {
        currentMonthTitle = month.title || null;
