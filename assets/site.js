  ...
  function renderMonthBody(monthId){
    ...
    for (var w = 1; w <= 4; w++){
      ...
      cache.weekly.forEach(function(quiz){
        ...
        var shareBtnHtml = quiz.result ? '<button class="jl-mp-share-btn" data-quiz-title="' + mpEsc(quiz.title) + '" data-score="' + quiz.result.score + '" data-total="' + quiz.result.total + '" title="شارك نتيجتك">📤</button>' : '';
        rows +=
